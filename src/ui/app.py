...
if "indexed_files" not in st.session_state:
    st.session_state.indexed_files = None  # None = not yet fetched

if "delete_confirm" not in st.session_state:
    st.session_state.delete_confirm = None  # filename pending confirmation
...
def list_files() -> list:
    r = requests.get(f"{API_BASE}/list", timeout=10)
    r.raise_for_status()
    return r.json().get("files", [])


def delete_file(file_name: str) -> dict:
    r = requests.post(f"{API_BASE}/delete", params={"file_name": file_name}, timeout=30)
    r.raise_for_status()
    return r.json()
...
with st.sidebar:
        ...
        if st.button("⬆️ Ingest Documents", use_container_width=True, type="primary"):
            with st.spinner("Ingesting…"):
                try:
                    ...
                    # Refresh the file list after ingestion
    ...
    col_refresh, col_count = st.columns([3, 2])

    with col_refresh:
        if st.button("🔄 Refresh List", use_container_width=True):
            with st.spinner("Fetching…"):
                try:
                    st.session_state.indexed_files = list_files()
    ...
    if files is None:
        st.caption("Click **Refresh List** to load indexed documents.")
    elif len(files) == 0:
        st.caption("No documents indexed yet.")
    else:
        with col_count:
            st.markdown(
                f'<p style="color:#a6e3a1;font-size:0.8rem;margin-top:6px;">'
        ...
        for file_info in files:
            ...
            with col_name:
                st.markdown(
                    f'<div class="file-name" title="{fname}">📄 {fname}</div>',
                    unsafe_allow_html=True,
                )

            with col_btn:
                # Two-step delete: first click arms confirmation, second confirms
                if st.session_state.delete_confirm == fname:
                    if st.button("✅ Yes", key=f"confirm_{fname}", use_container_width=True):
        ...
        # Show a cancel option when a deletion is armed
        if st.session_state.delete_confirm:
            st.warning(
                f"⚠️ Delete **{st.session_state.delete_confirm}**? "
